...
foreach ($ROW in $DATA_SPREADSHEET) 
{
	...
	#Copy AD group membership from template user to new user
	$CopyFromUser = Get-ADUser $templateUsername -Properties MemberOf
	$CopyToUser = Get-ADUser $newUsername -Properties MemberOf
	...
	if (!($createRDPandUpdateList[0] -eq 'n')) {
		...
		$content = Get-Content -Path 'COMPANY_RDS_REQUIRED-TEMPLATE.rdp'
		$newContent = $content -replace 'ADMIN', "$newUsername"
		$rdpFilename="RDS_$newUsername.rdp"
	...
	Write-Output "$firstName $lastName ($primaryEmail) created in AD and O365"
	if ($createRDPandUpdateList){
		Write-Output "$firstName $lastName ($primaryEmail) added to 'COMPANY_LIST' list in 'COMPANY_SHAREPOINT_TEAMS_SITE'"
		Write-Output "RDP file attached to their employee record and saved in the script directory. $LINE_BREAK_DASHES"
	}
	Write-Output "Apply the O365 license as indicated in the ticket. Standard is 'COMPANY_SPECIFIC' license, Microsoft Teams Standard, Microsoft Audio Conferencing."
	Write-Output "Add user to any explicitly requested AD security groups, O365 groups, shared mailboxes, or distribution lists. $LINE_BREAK_DASHES"
