#Connect to Exchange if user enters "Connect", otherwise assume an existing session is active
	...
	function MyGet-GroupNameAD {
		...
		while ($true) {
			...
			return $returnName
				...
				$searchString = Read-Host "Enter new $target_or_source search string:"
			}
		}
	}
...
While ($continueLoop) {
	...
	$newSourceSearchString = Read-Host "O365 SOURCE: enter search string (Defaults to previous source search string)"
