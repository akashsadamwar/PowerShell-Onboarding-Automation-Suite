...
{	
	...
	-Path $path2 `
	-Description "Created on $(Get-DateFormatted)"
	#END AD and sec group creation
	
	#START set variables for O365 and Teams
	$primarySmtpAddress = $alias+"@COMPANY1.com"
	...
	while ($continueLoop){	
		...
		if ($counter -eq 5) {
			Write-Output "$displayName team creation failed - Azure Group ID generation timed out."
			continue
		}
		if ($azureGroupId) {
